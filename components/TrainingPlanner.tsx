      ...
      if (athletesData) setAthletes(athletesData.map((a: any) => {
        ...
        return {
          ...
          injuries: (allInjuries || []).filter((i: any) => i.athlete_id === a.id).map((i: any) => ({
            ...
            returnDate: i.return_date, surgeryDate: i.surgery_date || null, notes: i.notes,
      ...
      <aside className="hidden md:flex flex-col w-52 shrink-0 bg-slate-900 sticky top-0 h-screen z-10">
        ...
        <div className="px-4 py-5 border-b border-white/[0.06]">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-[13px] font-semibold text-slate-100 tracking-tight">Training Planner</span>
          </div>
        </div>
        {/* Nav */}
          ...
          {role === 'Admin' && (
            <div className="mb-3">
              ...
              <select value={viewingAs} onChange={e => setViewingAs(e.target.value as Role)}
                ...
                {(['Admin','Coach','Physio','S&C'] as Role[]).map(r => <option key={r} value={r} className="bg-slate-900">{r}</option>)}
              </select>
            </div>
          )}
            ...
            <div className="bg-slate-900 w-60 h-full shadow-xl flex flex-col" onClick={e => e.stopPropagation()}>
              <div className="px-4 py-5 border-b border-white/[0.06]">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-[13px] font-semibold text-slate-100">Training Planner</span>
                </div>
              </div>
...
const EndOfDayReport = ({ athletes, setAthletes, teamStructure, date, onSaveEOD, onBack, saving, savedEodData }: any) => {
  ...
  const [injuryData, setInjuryData] = useState<any>({ bodyPart: 'Head', startDate: today, returnDate: '', surgeryDate: '', notes: '', event: 'Training', surface: '4G', contact: 'Contact' });
  ...
  const openAddInjury = (athleteId: string) => {
    ...
    setInjuryData({ bodyPart: 'Head', startDate: today, returnDate: '', surgeryDate: '', notes: '', event: 'Training', surface: '4G', contact: 'Contact' });
  ...
  const openEditInjury = (athleteId: string, inj: Injury) => {
    ...
    setInjuryData({ bodyPart: inj.bodyPart, startDate: inj.startDate, returnDate: inj.returnDate || '', surgeryDate: (inj as any).surgeryDate || '', notes: inj.notes || '', event: (inj as any).event || 'Training', surface: (inj as any).surface || '4G', contact: (inj as any).contact || 'Contact' });
    setShowInjuryModal(true);
  };
...
const [photo, setPhoto] = useState(athlete?.photo || '');
  ...
  const saveInjury = () => {
    if (injuryData.bodyPart && injuryData.startDate) {
      ...
      setInjuryData({ bodyPart: 'Head', startDate: '', returnDate: '', surgeryDate: '', notes: '', event: 'Training', surface: '4G', contact: 'Contact' });
      setEditingInjuryId(null);
